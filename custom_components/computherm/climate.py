...
class ComputhermClimate(ClimateEntity, RestoreEntity):
    ...
    def __init__(self, hass, config):
        ...
        self._heating_cooling = BROADLINK_HEATING
        self._child_lock = BROADLINK_LOCK_OFF
        self._room_temp_adj = None
        self._fre = None
        self._osv = None
        self._sensor = None
    ...
    def extra_state_attributes(self) -> dict:
        ...
        return {
            ...
            'heating_cooling': 'cool' if self._heating_cooling == BROADLINK_COOLING else 'heat',
            'child_lock': self._child_lock == BROADLINK_LOCK_ON,
            'hysteresis': self._hysteresis,
            'room_temp_adj': self._room_temp_adj,
            'anti_freeze': self._fre == 1,
            'osv': self._osv,
            'sensor_mode': self._sensor
        }
    ...
    async def async_set_temperature(self, **kwargs) -> None:
        ...
        if kwargs.get(ATTR_TEMPERATURE) is not None:
            ...
            if device.auth():
                device.set_power(BROADLINK_POWER_ON, self._child_lock, self._heating_cooling)
    ...
    async def async_set_hvac_mode(self, hvac_mode) -> None:
        ...
        if device.auth():
            if hvac_mode == HVACMode.OFF:
                device.set_power(BROADLINK_POWER_OFF, self._child_lock, self._heating_cooling)
            else:
                heating_cooling = self._heating_cooling
                if hvac_mode == HVACMode.COOL:
                    heating_cooling = BROADLINK_COOLING
                elif hvac_mode == HVACMode.HEAT:
                    heating_cooling = BROADLINK_HEATING

                device.set_power(BROADLINK_POWER_ON, self._child_lock, heating_cooling)
    ...
    async def async_set_child_lock(self, child_lock: bool) -> None:
        ...
        self._child_lock = BROADLINK_LOCK_ON if child_lock else BROADLINK_LOCK_OFF

        device = self._thermostat.device()
        if device.auth():
            current_power = BROADLINK_POWER_ON if self._thermostat_current_mode != HVACMode.OFF else BROADLINK_POWER_OFF
            device.set_power(current_power, self._child_lock, self._heating_cooling)
    ...
    async def async_update(self) -> None:
        ...
        self._room_temp_adj = data.get('room_temp_adj')
        self._fre = data.get('fre')
        self._osv = data.get('osv')
        self._sensor = data.get('sensor')
        ...
        # Heating/cooling mode and child lock
        self._heating_cooling = data.get('heating_cooling', BROADLINK_HEATING)
        self._child_lock = data.get('remote_lock', BROADLINK_LOCK_OFF)
        ...
        if data["power"] == BROADLINK_POWER_OFF:
            ...
            if data["auto_mode"] == BROADLINK_MODE_MANUAL or data['temp_manual'] == BROADLINK_TEMP_MANUAL:
                if self._heating_cooling == BROADLINK_COOLING:
                    self._thermostat_current_mode = HVACMode.COOL
                else:
        ...
        _LOGGER.debug(
            "Thermostat %s action=%s mode=%s heating_cooling=%s child_lock=%s",
            self._name, self._thermostat_current_action, self._thermostat_current_mode,
            'cool' if self._heating_cooling == BROADLINK_COOLING else 'heat',
            self._child_lock
        )
